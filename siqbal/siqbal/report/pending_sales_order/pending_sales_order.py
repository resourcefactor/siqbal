...
def execute(filters=None):
	...
	gl_entries_query = """
		...
		where voucher_no != against_voucher and so.transaction_date between '{0}' and '{1}'
		and so.company = '{2}' and so.docstatus=1 and so.status not in ('Closed') {3}
		...
		and so.company = '{2}' and so.docstatus=1 and so.status not in ('Closed') {3}
		and gle.is_cancelled = 0
	""".format(filters.fdate, filters.tdate, filters.company, customer_filter)
	print(gl_entries_query)

	gl_entries = frappe.db.sql(gl_entries_query, as_dict=1)
	order_gle = {}
