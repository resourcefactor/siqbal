...
def execute(filters=None):
    ...
    # Set default customer group if not provided
    if not filters.get("customer_group"):
        filters["customer_group"] = "Cash Customer"
    
    user = frappe.session.user
    filters["user"] = user
    
    # Build customer filter safely
    customer_filter = ""
    if filters.get("customer_group"):
        customer_filter = """ and so.customer_group in (select name from `tabCustomer Group`
                        where lft>=(select lft from `tabCustomer Group` where name = %(customer_group)s)
            and rgt<=(select rgt from `tabCustomer Group` where name = %(customer_group)s ))"""
    ...
    gl_query = """
        ...
        and so.company = %(company)s and so.docstatus = 1 and gle.is_cancelled = 0 and so.status not in ('Closed') {0}
        ...
        where voucher_no != against_voucher and gle.is_cancelled = 0 and so.transaction_date between %(fdate)s and %(tdate)s
        and so.company = %(company)s and so.docstatus = 1 and so.status not in ('Closed') {0}
    """.format(customer_filter)

    gl_entries = frappe.db.sql(gl_query, filters, as_dict=1)
