...
siqbal.SMSManager = function SMSManager(doc) {
	var me = this;
	this.setup = function() {
		var default_msg = {
			'Lead'				: '',
			...
			'Sales Order'		: `DEAR ${doc.customer_name} YOUR SALE ORDER ${doc.name} HAS BEEN CREATED

THANKS
FOR SHOPPING WITH US VISIT US AGAIN SOON`,

			'Delivery Note'		: 'Items has been delivered against delivery note: ' + doc.name
						+ (doc.po_no ? (' for your PO: ' + doc.po_no) : ''),
			'Sales Invoice': `DEAR ${doc.customer_name} YOUR SALE INVOICE ${doc.name} HAS BEEN CREATED
...
FOR SHOPPING WITH US VISIT US AGAIN SOON`,
	...
	this.show = function(contact, ref_doctype, ref_name, mobile_nos, message) {
		...
		frappe.db.get_doc('Customer', doc.customer).then(pr_doc => {
            me.number = pr_doc.cust_phone_number;
			me.show_dialog();
        });
//		if (mobile_nos) {
