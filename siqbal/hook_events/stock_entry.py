...
# 						if user_warehouse not in d.s_warehouse or user_warehouse.replace("Normal", "Breakage") not in d.s_warehouse or user_warehouse.replace("Depot", "Breakage") not in d.s_warehouse:
# 							frappe.throw(_("you are note allowed to tranfer stock from warehouse {0} .").format(d.s_warehouse))

# 				if d.s_warehouse:
# 					if strTransit in d.s_warehouse:
# 						if user_warehouse not in d.t_warehouse or user_warehouse.replace("Normal", "Breakage") not in d.t_warehouse or user_warehouse.replace("Depot", "Breakage") not in d.t_warehouse:
...
def validate_user_warehouse(se, method):
    ...
    if int(allow_all_warehouses) == 0 and not se.bypass_transit_warehouse:
        user_warehouse = frappe.db.get_value("User", {"name": frappe.session.uallow_all_warehousesser}, "user_warehouse")
        strTransit = "Transit"
        if user_warehouse:
            for d in se.get("items"):
                if d.t_warehouse:
