...
def update_child_qty_rate(parent_doctype, trans_items, parent_doctype_name, child_docname="items"):
    ...
    # SQM Calculation Logic (server-side equivalent of JS CalculateSQM)
    ...
    for d in data:
        ...
        # --- Server-side version of JS CalculateSQM using live values from Item master ---
        # That part of code only works for Purchase Order    
        if parent_doctype == "Purchase Order":
            def_boxes, def_pieces = frappe.db.get_value("Item", d.get("item_code"), ["boxes", "pieces"]) or (0, 0)
            qty = flt(d.get("qty"))
            boxes = flt(d.get("boxes"))
            pieces = flt(d.get("pieces"))
            sqm = qty  # default fallback

            if def_boxes > 0 and def_pieces > 0:
                # Determine total_piece based on field like in JS
                total_piece = round(qty / (def_boxes / def_pieces))
                sqm = round(total_piece * (def_boxes / def_pieces), 4)
                boxes = round(sqm / def_boxes)
                pieces = total_piece % def_pieces
                # Also override the qty with recalculated sqm (just like JS sets qty)
                d["qty"] = sqm
            else:
                # Fallback: assign all to qty
                sqm = qty
                boxes = qty
                pieces = qty

            # Assign updated values to the child item
            child_item.sqm = sqm
            child_item.boxes = boxes
            child_item.pieces = pieces
        else:
            # For other document types, use real core values (original calculation)
            child_item.sqm = flt(d.get("sqm", 0))  # Original SQM value
            child_item.boxes = flt(d.get("boxes", 0))  # Original Boxes value
            child_item.pieces = flt(d.get("pieces", 0))  # Original Pieces value
            child_item.qty = flt(d.get("qty", 0))  # Original Qty value
