...
# Dynamically create purchase invoice against sales taxes
def create_purchase_invoices_against_sales_taxes(si, method):
    siqbal_settings = frappe.get_single("SIqbal Settings")
    # Get all child table from siqbal settings child table name taxes_purchase_invoice
    if siqbal_settings.enable_auto_purchase_invoice_creation:
        for siqbal_row in siqbal_settings.taxes_purchase_invoice:
            if si.update_stock:
                sales_order_no = ""
                ...
                for row in si.taxes:
                    if siqbal_row.account in row.account_head:
