...
def get_item_details(args):
	...
	custom_uom_info = frappe.get_all("Item", fields=["name", "boxes", "pieces"], filters=[["name", "in", item_list]])
	...
	doc.update(
		{
			...
			"deductions": [{"account": deduction_account, "amount": (salaryslip.rounded_total - (total_paid_amount if total_paid_amount else 0))}]
		}
	)
